...
//
// The Python and C++ DP solvers independently implement the same Bellman
// recursion and CDF-bin transition kernel. A single hand-verified spot check
// (V(24,0) = 0.3760 in both) is weak evidence of parity -- it says nothing
// about the other ~8800 grid cells, and gives no warning if a future change
// to one side's math (transition kernel, terminal condition, risk-adjustment)
// isn't mirrored on the other.
//
// tests/cpp_backtest/tests/golden/dp_grid_golden.csv is generated by
// tests/cpp_backtest/tests/golden/generate_golden.py from the Python exact_dp solver using
// the same canonical parameters as this test. `make golden` regenerates it;
// CI regenerates it fresh and diffs against the committed copy to catch
// staleness, then runs this test to catch cross-language drift.
static void test_golden_parity_with_python() {
    std::printf("\n[Golden parity: C++ solve_dp vs Python exact_dp.solve]\n");

    std::ifstream in("tests/golden/dp_grid_golden.csv");
    if (!in) {
        CHECK(false, "golden/dp_grid_golden.csv found (run `make golden` to generate it)");
        return;
    }
    ...
    int n_tau, n_d, k_live;
    float dt_min, d_min, d_step, q_other, risk_aversion, sigma, mu;
    char comma;
    hs >> n_tau >> comma >> n_d >> comma >> dt_min >> comma >> d_min >> comma
       >> d_step >> comma >> q_other >> comma >> risk_aversion >> comma
       >> k_live >> comma >> sigma >> comma >> mu;

    // Reconstruct the SternModel from (sigma, mu) rather than (sigma, spread)
    // so this test is exact even if the spread->mu convention ever changes.
    SternModel stern(sigma, 0.0);
    stern.mu = mu;
    BidModel bm;   // default params — matches Python BidModel()'s HaircutParams defaults

    const float d_max = -d_min;
    DPGrid grid = solve_dp(stern, bm, n_tau > 0 ? (n_tau - 1) * dt_min : 48.0f,
                          dt_min, d_max, d_step, q_other, risk_aversion, k_live);

    CHECK(grid.n_tau == n_tau, "golden grid n_tau matches solve_dp() n_tau");
    CHECK(grid.n_d   == n_d,   "golden grid n_d matches solve_dp() n_d");

    double max_abs_diff = 0.0;
    int64_t exercise_mismatches = 0;
    int64_t cells_checked = 0;

    std::string line;
    for (int idx = 0; idx < n_tau * n_d && std::getline(in, line); ++idx) {
        std::stringstream ls(line);
        double golden_value;
        int golden_exercise;
        ls >> golden_value >> comma >> golden_exercise;

        const double cpp_value = static_cast<double>(grid.values[idx]);
        max_abs_diff = std::max(max_abs_diff, std::abs(cpp_value - golden_value));
        if (grid.sell_here[idx] != static_cast<bool>(golden_exercise))
            ++exercise_mismatches;
        ++cells_checked;
    }

    std::printf("    cells checked: %lld / %d\n", (long long)cells_checked, n_tau * n_d);
    std::printf("    max |V_cpp - V_python| = %.6f\n", max_abs_diff);
    std::printf("    exercise-boundary mismatches: %lld\n", (long long)exercise_mismatches);

    CHECK(cells_checked == static_cast<int64_t>(n_tau) * n_d,
          "golden file has one row per grid cell");
    // float32 accumulation over ~96 backward steps vs Python's float64; 1e-3
    // is comfortably above observed drift (~5e-5) and well below anything
    // that would matter for a SELL/HOLD decision.
    CHECK(max_abs_diff < 1e-3, "V grid matches Python within 1e-3 on every cell");
    // Boundary ties (node_bid == E_cont to floating precision) can flip a
    // handful of cells' sell/hold call without indicating real drift.
    CHECK(exercise_mismatches < cells_checked / 200,
          "exercise boundary matches Python on >99.5% of cells");
}
...
int main() {
    ...
    test_golden_parity_with_python();
