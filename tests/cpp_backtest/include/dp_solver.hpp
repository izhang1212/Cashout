                       ...
                       int   k_live       = 2)
{
    ...
    // ── τ = 0: terminal values. Matches Python's exercise[-1] = True: at the
    // buzzer there's no more continuation to hold for, so the position always
    // settles here (mirrors exact_dp.py's terminal condition exactly). ──────
    ...
    for (int ti = 1; ti < n_tau; ++ti) {
        ...
        for (int di = 0; di < n_d; ++di) {
            ...
            // >= (not >) matches Python exact_dp.py's `node_bids >= cont` --
            // ties (bid == cont, e.g. both pinned to 0 in the always-losing
            // tail) must resolve the same way on both sides.
