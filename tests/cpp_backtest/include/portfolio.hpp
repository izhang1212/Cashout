...
struct StrategyStats {
    ...
    // "Loss" = realized P&L < 0, regardless of how the position was exited.
    // Defined purely on terminal P&L so the metric is comparable across strategies.
    // (The old "bust = !sold_early && !combo_won" was not apples-to-apples: it
    // excluded dead-combo games where the DP exited early at bid≈0 but P&L = -entry,
    // identical to holding — making identical P&L distributions show different bust%.)
    float loss_rate() const {
        if (results.empty()) return 0.0f;
        int b = 0;
        for (auto& r : results) if (r.pnl < 0.0f) ++b;
