...
def synthetic_game_ticks(stern: SternModel, bid_model: BidModel, *,
    ...
    # Resolve "other legs" once at the end with prob q_other(0).
    other_hit = rng.random() < q_other_fn(0.0)
    ml_won = sign * path[-1] > 0
    combo_won = int(ml_won and other_hit)

    ticks = []
    for i, d in enumerate(path):
        tau = tau0 * (1 - i / n_steps)
        p_ml = stern.win_prob(d, tau, side=moneyline_side)
        p_combo = p_ml * q_other_fn(tau)
