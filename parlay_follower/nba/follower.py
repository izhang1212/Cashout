...
class LiveFollower:
    ...
    def _update_mu_from_market(self, p_home_market: float,
        ...
        # Phi^-1(P) = (d + mu * tau) / (sigma * sqrt(tau))  =>  mu = ...
        mu_implied = (z * self.stern.sigma * np.sqrt(tau) - score_diff) / tau
        # Clip to ±1.0 (equivalent to a ~50-pt pregame spread) to reject outliers
        mu_implied = float(np.clip(mu_implied, -1.0, 1.0))

        # Ramp up live weight as more game-time elapses
        w_live = min(elapsed_min / (elapsed_min + _MU_RAMP_HALFLIFE_MIN),
                     _MU_LIVE_MAX_WEIGHT)
        old_mu = self.stern.mu
        self.stern.mu = w_live * mu_implied + (1.0 - w_live) * self._pregame_mu
