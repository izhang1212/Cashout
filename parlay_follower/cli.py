...
def cmd_positions(_args):
    ...
    from .execution.config import base_url, load_creds, load_settings
...
def _parse_legs(leg_specs: list[str]) -> list:
    from .data_gathering.nba.game_state import Leg
    legs = []
...
def cmd_historical_backtest(args):
    """Run the decision policy on real historical NBA games and compare vs benchmarks."""
    from tests.backtest.historical_replay import (
        historical_policy_comparison, pull_nba_games, nba_ticks_from_pbp,
    )
    from tests.backtest.metrics import summarize
...
def main():
    ...
    hb = sub.add_parser("historical-backtest",
                         help="Run decision policy on real historical NBA games")
    ...
    hb.add_argument("--spread", type=float, default=-4.5,
                    help="Pregame spread used at entry (default -4.5)")
    ...
    fl.add_argument("--game-id", required=True, help="nba_api game id")
    fl.add_argument("--spread", type=float, default=None,
                    help="pregame home line (e.g. -4.5)")
